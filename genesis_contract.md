...
- actions: 
  1. 创建验证者；
  2. 如果交易的value值超过`10000CET`，则将同步完成质押操作；
- params: 
...
- conditions: 
  ...
  3. 你质押到该验证者的数量大于0。
- actions: 
  1. 解除给某个验证者的质押，如果该验证者的总质押数量小于`10000CET`，将其从候选出块节点集合中踢出。
- params: 
    | name | type | required | description |
    | :--- | :--- | :--- | :--- |
    | validator | address | yes | 验证节点地址 |
...
- actions: 
  ...
  2. 如果该验证者的总质押数量超过`10000CET`，将其加入候选出块节点集合。
- params: 
- result: 
    | name | type | required | description |
    | :--- | :--- | :--- | :--- |
